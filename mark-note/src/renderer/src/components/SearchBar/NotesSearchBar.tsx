...
export const NotesSearchBar = ({onSelect, onChange, ...props}: NoteSearchBarProps) => {
    ...
    const {allNotes, handleNotesFiltered} = useNotesList({onSelect})
    ...
    placeholder="Search Notes" onChange={event => setQuery(event.target.value)} onKeyUp={handleNotesFiltered(query)}/>
    </div>
    )
}